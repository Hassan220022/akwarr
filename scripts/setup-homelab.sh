...
BANDWIDTH_TEST_URL="${BANDWIDTH_TEST_URL:-https://speed.cloudflare.com/__down?bytes=25000000}"
BANDWIDTH_TEST_SECONDS="${BANDWIDTH_TEST_SECONDS:-30}"
BANDWIDTH_MEASURED_BYTES_PER_SECOND="${BANDWIDTH_MEASURED_BYTES_PER_SECOND:-}"
if [[ -z "${ARIA2_MAX_OVERALL_DOWNLOAD_LIMIT:-}" ]]; then
  if [[ -z "${BANDWIDTH_MEASURED_BYTES_PER_SECOND}" ]]; then
    log "Measuring current WAN download speed for aria2 cap"
    BANDWIDTH_MEASURED_BYTES_PER_SECOND="$(
      measure_download_bytes_per_second "${BANDWIDTH_TEST_URL}" "${BANDWIDTH_TEST_SECONDS}"
    )"
  fi
  BANDWIDTH_MEASURED_MBIT="$(mbit_from_bytes_per_second "${BANDWIDTH_MEASURED_BYTES_PER_SECOND}")"
  ARIA2_MAX_OVERALL_DOWNLOAD_LIMIT="$(
    aria2_limit_from_bytes_per_second \
      "${BANDWIDTH_MEASURED_BYTES_PER_SECOND}" \
      "${ARIA2_BANDWIDTH_LIMIT_PERCENT}"
  )"
else
  BANDWIDTH_MEASURED_MBIT="${BANDWIDTH_MEASURED_MBIT:-manual}"
fi
...
BANDWIDTH_TEST_URL=${BANDWIDTH_TEST_URL}
BANDWIDTH_TEST_SECONDS=${BANDWIDTH_TEST_SECONDS}
BANDWIDTH_MEASURED_BYTES_PER_SECOND=${BANDWIDTH_MEASURED_BYTES_PER_SECOND}
BANDWIDTH_MEASURED_MBIT=${BANDWIDTH_MEASURED_MBIT}
